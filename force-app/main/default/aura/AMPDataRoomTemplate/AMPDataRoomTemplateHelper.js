({
    ...
    divspinner:'',
    body:'',
	bindDataroom : function(component,event,helper) {
        $(helper.body).hide();
    ...
    leftnav: function(component,event,helper,classname)
    {
     	...
     	if(currentNav == "require")
        {
            ...
            component.set("v.templatetype","36864");
        ...
        helper.bindDataroom(component,event,helper);
    },
    ...
    send: function(component,event,helper)
    {
        ...
        var rec = $.parseJSON(JSON.stringify(component.get("v.dataroom"))).items[index];
        //if(canextplaybookSend)
        {
            var templatedata = $.parseJSON(rec.metadata);
            templatedata["isSmartlist"]=null;
            templatedata["linkids"]=rec.linkid;
    ...
    customize: function(component,event,helper)
    {
        ...
        if(!templatedata.output)
        {
            if(userdata.canCreateAssets)
            {
                ...
                var url = install+'/#v4u/ajax/widgets/sf-handler.cshtml?type=DATAROOM&sftask=CreateTemporaryMedia&id='+templatedata.id+'&handler=dataroom&name='+name+'&usedatasource='+templatedata.usedatasource + '&tpapp=sfdc&clean';
                helper.showEnablement(helper,component,url);
            }
        }
        else if(userdata.canCreateAssets)
        {
            ...
            templatedata["canedit"] = userdata.caneditAsset;
            templatedata["linkids"] = linkid;
            templatedata["isSmartlist"] = isSmartlist;
            var _templatedata = window.btoa((JSON.stringify(templatedata)));
